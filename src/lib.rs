...
impl<T: Serialize + DeserializeOwned + Unpin> Sink<T> for OwnedWriteHalfTyped<T> {
    ...
    fn poll_flush(mut self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Result<(), Self::Error>> {
        match self.as_mut().maybe_send(cx, false) {
            Poll::Ready(Ok(Some(()))) => {
    ...
    fn poll_close(mut self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Result<(), Self::Error>> {
        match self.as_mut().maybe_send(cx, true) {
            Poll::Ready(Ok(Some(()))) => {
    ...
    fn maybe_send(
        ...
        loop {
            return match state {
                ...
                WriteHalfState::Closing => match Pin::new(&mut *raw).poll_write(cx, &[0]) {
                    Poll::Ready(Ok(len)) => {
                        if len == 1 {
                            *state = WriteHalfState::Closed;
                            Poll::Ready(Ok(Some(())))
                        } else {
                            continue;
                        }
                ...
                WriteHalfState::Closed => Poll::Ready(Ok(None)),
            };
        }
    }
}
...
impl<T: Serialize + DeserializeOwned + Unpin> Drop for OwnedWriteHalfTyped<T> {
    fn drop(&mut self) {
